            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        <label class="col-sm-2 control-label">qTitle</label>
                        <div class="col-sm-10">
                            <input type="text" name="qTitle" class="form-control" id="username_add_input" >
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">qMadeByUserId</label>
                        <div class="col-sm-10">
                            <input type="password" name="qMadeByUserId" class="form-control" id="password_add_input" >
                            <span class="help-block"></span>
                        </div>
                    </div>
                </form>
            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="id" class="form-control" id="userId_modify_input" value="${question.qId}" readonly placeholder="userid">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">qTitle</label>
                        <div class="col-sm-10">
                            <input type="text" name="username" class="form-control" id="username_modify_input" value="${question.qTitle}" placeholder="username">
                            <span class="help-block"></span>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">qMadeByUserId</label>
                        <div class="col-sm-10">
                            <input  name="password" class="form-control" id="password_modify_input" value="${question.qDetail}" placeholder="password">
                            <span class="help-block"></span>
                        </div>
                    </div>
                </form>
...
<script>
    ...
    function build_users_table(result){
        ...
        var questions = result.extend.pageInfo.list;//list<User>users
        //index：下标 item：单个对象
        $.each(questions,function(index,question){
            var checkBoxTd = $("<td><input type='checkbox' name='check-id' value="+question.qId+" class='check_item'/></td>");
            var qId = $("<td></td>").append(question.qId);
            var qTitle = $("<td></td>").append(question.qTitle);
            var qDetail = $("<td></td>").append(question.qDetail);
            var qMadeByUserId = $("<td></td>").append(question.qMadeByUserId);
            var qMadeDate = $("<td></td>").append(new Date(parseInt(question.qMadeDate)).toLocaleDateString());
            ...
            editBtn.attr("edit-id",question.qId);
            ...
            delBtn.attr("del-id",question.qId);
    ...
    $("#user_delete_all_btn").click(function (){
        ...
        //alert(ids)
        $.ajax({
            url:"/YourAnswer/questionBatchDelete",
            type:"POST",
            data:{"ids":ids},
            success:function (result) {
                if(result.code==100){
    ...
    $(document).on("click",".delete_btn",function(){
        ...
        var qId = $(this).attr("del-id");

        if(confirm("确认删除【"+uName+"】吗？")){
            //确认，发送ajax请求删除即可
            $.ajax({
                url:"/YourAnswer/questionDelete",
                type:"POST",
                data:{"id":qId},
                success:function(result){
                    //alert(result.msg);
    ...
    $(document).on("click",".edit_btn",function(){
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                $("#userId_modify_input").val(result.extend.question.qId);
                $("#username_modify_input").val(result.extend.question.qTitle);
                $("#password_modify_input").val(result.extend.question.qDetail);
            }});
        //2、弹出模态框
    ...
    $("#user_modify_btn").click(function(){
        $.ajax({
            ...
            type:"POST",
            data:{"qId":$("#userId_modify_input").val(),
                "qTitle":$("#username_modify_input").val(),
                "qMadeByUserId":$("#password_modify_input").val()
            },
            success:function(result){
                //alert(result.msg);
                if(result.code == 100){
